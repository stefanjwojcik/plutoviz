...
df = CSV.read("/home/swojcik/Downloads/data_master_1.csv", DataFrame)
df[:, :sp_500]
...
# Dickey-Fuller test (h_0 is that the series is non-stationary)
...
# Dickey-Fuller test (h_0 is that the series is non-stationary) - should be stationary now
...
sampler = NUTS(1000, .65) # only 1000 samples 
posterior_y = sample(ARIMA110(s_train), sampler, 100)

plot(chain_ARIMA110, reuse = false, title = "Sampler plot for ARIMA(1, 1, 0) model")
gui()

# ARima of order (0,1,1)
